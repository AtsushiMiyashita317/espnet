...
class VariationalFastSpeechGW(AbsTTS):
    ...
    def forward(
        ...
        # forward propagation
        before_outs, after_outs, d_outs, p_outs, ps, e_outs, es = self._forward(
            xs,
            ilens,
            ys,
            olens,
    ...
    def _forward(
        ...
        if is_inference:
            ...
            _, map_inv = self.length_regulator(torch.zeros_like(ds).unsqueeze(-1), -ds, is_inference)  # (B, T_feats, adim)      
