...
class FastSpeechGW(AbsTTS):
    ...
    def __init__(
        ...
        duration_predictor_lam: float = 1e-10,
        # energy predictor
        ...
        Args:
            ...
            use_weighted_masking (bool): Whether to apply weighted masking in loss
                calculation.
            lr_window_size (int): Window size for STGW
            lr_n_iter (int): The number of iterations for STGW
            lr_mode (str): If 'after', warp follows interpolate. If 'before', 
                interpolate follows warp.
        """
        ...
        self.duration_predictor = AlignmentModule(
            tdim=idim,
            fdim=odim,
    ...
    def _forward(
        ...
        d_outs = self.duration_predictor(xs, ys, text_masks, feat_masks)  # (B, T_text)
    ...
    def inference(
        ...
        """
        x, y = text, feats
        spemb, d, p, e = spembs, None, pitch, energy
...
class VariationalFastSpeechGW(AbsTTS):
    def __init__(
        self,
        # network structure related
        idim: int,
        ...
        # define criterions
        self.criterion = VariationalFastSpeechGWLoss(
            use_masking=use_masking, 
            use_weighted_masking=use_weighted_masking,
            lr_mode='after',
            lr_n_fft=lr_n_fft
    ...
    def forward(
        ...
        loss = l1_loss + 0*duration_loss + pitch_loss + energy_loss
    ...
    def _forward(
        ...
        ws = self.alignment_module(hs, ys, feat_masks)  # (B, T_text, 2)
        # n = ws.size(-2)
        ...
        # ds = self.istft(ws, n)
        ds = ws
        ds = ds.masked_fill(feat_masks.unsqueeze(-1), 0.0)
        ds = torch.nn.functional.pad(ds, [0,0,1,0])[...,:-1,:]
        ds = ds - ds.sum(-2, keepdim=True)/olens.unsqueeze(-1).unsqueeze(-1)
        ds = ds.cumsum(-2)
        ds = ds.masked_fill(feat_masks.unsqueeze(-1), 0.0)
        
        hs, func = self.length_regulator(hs, ds, is_inference)  # (B, T_feats, adim)
    ...
    def inference(
        ...
        map = gw.cubic_interpolation(torch.eye(d_outs.size(-1), device=d_outs.device).unsqueeze(0), d_outs.detach()).transpose(-1,-2)
        map = map.detach().cpu()
...
class VNART(AbsTTS):
    ...
    def forward(
        ...
        # forward propagation
        before_outs, after_outs, enc_mu, enc_in_var, dec_mu, dec_ln_var = self._forward(
            xs,
            ilens,
            ys,
            olens,
        ...
        # calculate loss
        l1_loss, kl_loss = self.criterion(
            after_outs=after_outs,
            before_outs=before_outs,
            ys=ys,
            ds=ds,
    ...
    def _forward(
        ...
        if is_inference:
            ...
            hs = gw.utils.interpolate(hs, ilens, olens)
            zs, _, _, trc_mu2, trc_ln_var2= self.tracer2.forward(hs, None, x_masks, is_inference=True)
        else:
            hs, _, enc1 = self.encoder1.forward_one_step(ys, y_masks)
            hs, _, enc2 = self.encoder2.forward_one_step(hs, y_masks)
            
            hs, _, dec1, dec_mu1, dec_ln_var1 = self.decoder1.forward(xs, x_masks, enc2, y_masks)
            hs = gw.utils.interpolate(hs, ilens, olens)
            zs, _, dec2, dec_mu2, dec_ln_var2 = self.decoder2.forward(hs, x_masks, enc1, y_masks)
            
            hs, _, trc1, trc_mu1, trc_ln_var1= self.tracer1.forward(xs, dec1, x_masks)
            hs = gw.utils.interpolate(hs, ilens, olens)
            hs, _, trc2, trc_mu2, trc_ln_var2= self.tracer2.forward(hs, dec2, x_masks)
        ...
        return (
            ...
            after_outs, 
            dec_mu1+dec_mu2,
            dec_ln_var1+dec_ln_var2,
            trc_mu1+trc_mu2,
            trc_ln_var1+trc_ln_var2
        )
