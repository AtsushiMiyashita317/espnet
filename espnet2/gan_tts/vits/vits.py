...
from espnet2.gan_tts.vits.generator import VITSGenerator, VITSGWGenerator
from espnet2.gan_tts.vits.loss import KLDivergenceLoss
from espnet2.torch_utils.device_funcs import force_gatherable

AVAILABLE_GENERATERS = {
    "vits_generator": VITSGenerator,
    "vits_gw_generator": VITSGWGenerator,
}
...
class VITSGW(AbsGANTTS):
    ...
    def __init__(
        ...
        odim: int,
        sampling_rate: int = 22050,
        generator_type: str = "vits_gw_generator",
        generator_params: Dict[str, Any] = {
            "hidden_channels": 192,
            "spks": None,
            "langs": None,
            ...
            "posterior_encoder_dropout_rate": 0.0,
            "use_weight_norm_in_posterior_encoder": True,
            "prior_encoder_kernel_size": 5,
            "prior_encoder_layers": 4,
            "prior_encoder_stacks": 1,
            "prior_encoder_base_dilation": 1,
            "prior_encoder_dropout_rate": 0.0,
            "use_weight_norm_in_prior_encoder": True,
            "warping_predictor_kernel_size": 5,
            "warping_predictor_layers": 4,
            "warping_predictor_stacks": 1,
            "warping_predictor_base_dilation": 1,
            "warping_predictor_dropout_rate": 0.0,
            "use_weight_norm_in_warping_predictor": True,
            "flow_flows": 4,
            "flow_kernel_size": 5,
            "flow_base_dilation": 1,
            "flow_layers": 4,
        ...
        # define modules
        generator_class = AVAILABLE_GENERATERS[generator_type]
        if generator_type == "vits_gw_generator":
            # NOTE(kan-bayashi): Update parameters for the compatibility.
            #   The idim and odim is automatically decided from input data,
            #   where idim represents #vocabularies and odim represents
            #   the input acoustic feature dimension.
    ...
    def _forward_generator(
        ...
        # parse outputs
        speech_hat_, start_idxs, _, z_mask, outs_s, outs_v = outs
        _, s_q, s_m, s_logs, _, z_logs = outs_s
        _, v_q, v_m, v_logs, _, w_logs = outs_v
        speech_ = get_segments(
            x=speech,
            start_idxs=start_idxs * self.generator.upsample_factor,
            segment_size=self.generator.segment_size * self.generator.upsample_factor,
        ...
        with autocast(enabled=False):
            mel_loss = self.mel_loss(speech_hat_, speech_)
            kl_loss = self.kl_loss(s_q, z_logs, s_m, s_logs, z_mask)
            dur_loss = self.kl_loss(v_q, w_logs, v_m, v_logs, z_mask)
            adv_loss = self.generator_adv_loss(p_hat)
            feat_match_loss = self.feat_match_loss(p_hat, p)
