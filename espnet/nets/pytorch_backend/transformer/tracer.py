...
class Tracer(torch.nn.Module):
    ...
    def forward(self, xs, xs_cache, xs_masks, cache=None, is_inference=False):
        """Encode input frame.
        ...
        for layer_idx, encoder_layer in enumerate(self.encoders):
            input = xs if is_inference or layer_idx == 0 else xs_cache[layer_idx-1]
            xs, masks = encoder_layer(
                input, 
                xs_masks, 
                cache=cache[layer_idx]
            )
            xs, mu, ln_var = self.reparametrizers[layer_idx](xs)
            xs_cache.append(xs)
            mu_cache.append(mu)
