...
from espnet.nets.pytorch_backend.transformer.mixer_layer import MixerLayer
...
from espnet.nets.pytorch_backend.transformer.reparametrize import Reparametrize
...
class Mixer(torch.nn.Module):
    ...
    def __init__(
        ...
        grad_rate_stop=20000,
        grad_rate_warmup=50000
    ):
        ...
        self.reparametrizers = torch.nn.ModuleList([Reparametrize(attention_dim) for _ in range(num_blocks)])
        ...
        self.n_iter = torch.nn.Parameter(torch.tensor(0.0))
        self.n_iter.requires_grad = False
        self.stop = grad_rate_stop
        self.warmup= grad_rate_warmup
        
    def step(self):
        self.n_iter.data = self.n_iter.data + 1.0
        
    def grad_rate(self):
        return 0.0 if self.n_iter.item()<self.stop else (self.n_iter.item()-self.stop)/(self.warmup) if self.n_iter.item() < self.stop+self.warmup else 1.0
    ...
    def forward(self, xs, xs_masks, ys_cache, ys_masks, cache=None):
        """Encode input frame.
        ...
        if isinstance(
            ...
            xs, xs_masks = self.embed(xs, xs_masks)
        else:
            xs = self.embed(xs)
        if cache is None:
        ...
        mu_cache = []
        ln_var_cache = []
        masks = torch.logical_and(xs_masks.unsqueeze(-1), ys_masks.unsqueeze(-2)).squeeze(-3)
        for layer_idx, encoder_layer in enumerate(self.encoders):
            xs, masks = encoder_layer(
                xs, 
                ys_cache[-layer_idx-1], 
                masks, 
                cache=cache[layer_idx]
            )
            xs, mu, ln_var = self.reparametrizers[layer_idx](xs, grad_rate=self.grad_rate())
            xs_cache.append(xs)
            mu_cache.append(mu)
            ln_var_cache.append(ln_var)
        if self.normalize_before:
            xs = self.after_norm(xs)
        self.step()
        return xs, masks, xs_cache, mu_cache, ln_var_cache
